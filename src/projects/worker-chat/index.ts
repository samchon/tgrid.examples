...
async function fork(name: string, ...messages: string[]): Promise<WorkerConnector<ChatService>>
{
    ...
    let connector: WorkerConnector<ChatService> = new WorkerConnector(service);

    service.assign(connector.getDriver<IChatPrinter>());
    await connector.connect(__dirname + "/child.js", name, ...messages);
...
{
    let connectors: WorkerConnector<ChatService>[] = [];
